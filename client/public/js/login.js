...
function setRole(role) {
  ...
  if (role === 'admin') {
    // Admins don't need apartment number
    aptGroup.style.display = 'none';
    aptInput.removeAttribute('required');

    document.getElementById('login-sub').textContent   = 'Sign in to your admin account';
    document.getElementById('register-sub').textContent = 'Register as a new administrator';
    document.getElementById('login-title').innerHTML   = '<i class="fa-solid fa-shield-halved"></i> Admin Sign In';
    document.getElementById('register-title').innerHTML = '<i class="fa-solid fa-user-shield"></i> Admin Registration';
  } else {
    aptGroup.style.display = '';
    aptInput.setAttribute('required', '');
...
document.getElementById('register-form').addEventListener('submit', async function (e) {
  ...
  const body = { name, email, password, role: currentRole };
  if (currentRole === 'resident') body.apartmentNumber = apt;
